...
// load utility files
/* that awkward moment when you tell it to load game.js, creating a self sustaining hell-hole*/
...
function Loader() {
	...
	this.Load = function (assets) {
		for (var i in assets) {
			...
			if (!this.assetsLoading[assets[i]] && !this.assetsLoaded[assets[i]]) {
				var img = new Image();
				img.src = this.domainImg + assets[i];
				img.alt = assets[i];
	...
	this.LoadAsync = function (assets) {}

	this.LoadScript = function (script) {
		script = (typeof script == "object") ? script : [script];

		script.forEach((n,i)=>{script[i] = new Promise(function (resolve, reject) {
...
Game.LoadAssets = function () {
	...
	Game.Loader.domainImg = 'assets/img/';
	Game.Loader.domainJs = 'js/';
	//Game.Loader.loaded = Game.Init;
	//Game.Loader.Load(['filler.png']);

	// 			var fTLString = ["init/season.js", "building/object.js", "util/helper.js", "util/ajax.js", "util/base64.js"],
	// 	fTL = [];
	// fTLString.forEach((n, i) => {
	// 	fTL[i] = loadScript("js/" + n)
	// });
	// Promise.all(fTL).then((values) => {
	const PreInitFiles = ["js/init/season.js", "js/building/object.js", "js/util/helper.js", "js/util/ajax.js", "js/util/base64.js"];
	const InitFiles = ["js/preinit/presets.js"];
	const PostInitFiles = [];
	return new Promise(function (resolve, reject) {
	Game.Loader.LoadScript(PreInitFiles).then(() => { // PREINIT
		Game.Loader.LoadScript(InitFiles).then(() => { // INIT
			Game.Loader.LoadScript(PostInitFiles).then(() => { // POSTINIT
				Game.Launch();
				resolve();
			});
		});
...
Game.Launch = function () {
	...
	css.type = 'text/css';
	css.innerHTML = 'body .icon,body .crate,body .usesIcon{background-image:url(assets/img/icons.png?v=' + Game.version + ');}';
	...
	Game.Init = function () {
		Game.ready = 1;
		//loadScript("js/preinit/presets.js").then(() => {});
		...
		Game.attachTooltip = function (el, func, origin) {
			...
			origin = (origin ? origin : 'middle');
			AddEvent(el, 'mouseover', function (func, el, origin) {
				return function () {
		...
		Game.GrabData = function () {
			//ajax('http://cors-anywhere.herokuapp.com/http://orteil.dashnet.org/patreon/grab.php',Game.GrabDataResponse);
		}
		...
		AddEvent(l('httpsSwitch'), 'click', function () {
			PlaySound('assets/assets/snd/pop' + Math.floor(Math.random() * 3 + 1) + '.mp3', 0.75);
		...
		Game.attachTooltip(l('heralds'), function () {
			...
			str += '<div style="width:31px;height:39px;background:url(assets/img/heraldFlag.png);position:absolute;top:0px;left:8px;"></div><div style="width:31px;height:39px;background:url(assets/img/heraldFlag.png);position:absolute;top:0px;right:8px;"></div>';
		...
		Game.Reset = function (hard) {
			...
			for (var i in Game.UpgradesById) {
				...
				if (me.pool != 'prestige' && !me.lasting) {
					if (Game.Has('Keepsakes') && Game.seasonDrops.indexOf(me.name) != -1 && Math.random() < 1 / 5) {} else if (Game.ascensionMode == 1 && Game.HasAchiev('O Fortuna') && me.tier == 'fortune') {} else if (Game.HasAchiev('O Fortuna') && me.tier == 'fortune' && Math.random() < 0.4) {} else me.unlocked = 0;
				}
			}
		...
		l('ascendOverlay').innerHTML =
			...
			'<div class="ascendData smallFramed prompt" ' + Game.getTooltip(
				'<div style="min-width:300px;text-align:center;font-size:11px;">Each prestige level grants you a permanent <b>+1% CpS</b>.<br>The more levels you have, the more cookies they require.</div>', 'bottom-right') + ' style="margin-top:8px;"><h3 id="ascendPrestige"></h3></div>' +
			'<div class="ascendData smallFramed prompt" ' + Game.getTooltip(
				'<div style="min-width:300px;text-align:center;font-size:11px;">Heavenly chips are used to buy heavenly upgrades.<br>You gain <b>1 chip</b> every time you gain a prestige level.</div>', 'bottom-right') + '><h3 id="ascendHCs"></h3></div>' +
			'<a id="ascendButton" class="option framed large red" ' + Game.getTooltip(
				'<div style="min-width:300px;text-align:center;font-size:11px;">Click this once you\'ve bought<br>everything you need!</div>', 'bottom-right') + ' style="font-size:16px;margin-top:0px;"><span class="fancyText" style="font-size:20px;">Reincarnate</span></a>' +
		...
		Game.UpdateAscendIntro = function () {
			if (Game.AscendTimer == 1) PlaySound('assets/snd/charging.mp3');
			if (Game.AscendTimer == Math.floor(Game.AscendBreakpoint)) PlaySound('assets/snd/thud.mp3');
			Game.AscendTimer++;
			if (Game.AscendTimer > Game.AscendDuration) //end animation and launch ascend screen
			{
				PlaySound('assets/snd/cymbalRev.mp3', 0.5);
				PlaySound('assets/snd/choir.mp3');
		...
		Game.UpdateReincarnateIntro = function () {
			if (Game.ReincarnateTimer == 1) PlaySound('assets/snd/pop' + Math.floor(Math.random() * 3 + 1) + '.mp3', 0.75);
		...
		Game.harvestLumps = function (amount, silent) {
			...
			if (!silent) {
				...
				PlaySound('assets/snd/pop' + Math.floor(Math.random() * 3 + 1) + '.mp3', 0.75);
			}
			Game.computeLumpTimes();
		}
		...
		Game.spendLump = function (n, str, func) {
			...
			return function () {
				...
				if (Game.prefs.askLumps) {
					...
					Game.Prompt('<div class="icon" style="background:url(assets/img/icons.png?v=' + Game.version + ');float:left;margin-left:-8px;margin-top:-8px;background-position:' + (-29 * 48) + 'px ' + (-14 * 48) + 'px;"></div><div style="margin:16px 8px;">Do you want to spend <b>' + Beautify(n) + ' lump' + (n != 1 ? 's' : '') + '</b> to ' + str + '?</div>', [
		...
		Game.playCookieClickSound = function () {
			if (Game.prefs.cookiesound) PlaySound('assets/snd/clickb' + (Game.cookieClickSound) + '.mp3', 0.5);
			else PlaySound('assets/snd/click' + (Game.cookieClickSound) + '.mp3', 0.5);
		...
		Game.ClickCookie = function (e, amount) {
			...
			if (Game.OnAscend || Game.AscendTimer > 0 || Game.T < 3 || now - Game.lastClick < 1000 / 250) {} else {
		...
		Game.CalculateGains = function () {
			...
			else if (name == 'ortiel') mult *= 0.98; //or so help me god
		...
		Game.shimmer = function (type, obj, noCount) {
			...
			if (!Game.touchEvents) {
				AddEvent(this.l, 'click', function (what) {
					return function (event) {
						what.pop(event);
					};
				}(this));
			} else {
				AddEvent(this.l, 'touchend', function (what) {
					return function (event) {
						what.pop(event);
					};
				}(this));
		...
		Game.shimmerTypes = {
			...
			'golden': {
				...
				initFunc: function (me) {
					if (!this.spawned && Game.chimeType == 1 && Game.ascensionMode != 1) PlaySound('assets/snd/chime.mp3');

					//set image
					var bgPic = 'assets/img/goldCookie.png';
					var picX = 0;
					var picY = 0;
					...
					if ((!me.forceObj || !me.forceObj.noWrath) && ((me.forceObj && me.forceObj.wrath) || (Game.elderWrath == 1 && Math.random() < 1 / 3) || (Game.elderWrath == 2 && Math.random() < 2 / 3) || (Game.elderWrath == 3) || (Game.hasGod && Game.hasGod('scorn')))) {
						me.wrath = 1;
						if (Game.season == 'halloween') bgPic = 'assets/img/spookyCookie.png';
						else bgPic = 'assets/img/wrathCookie.png';
					} else {
						me.wrath = 0;
					}

					if (Game.season == 'valentines') {
						bgPic = 'assets/img/hearts.png';
						picX = Math.floor(Math.random() * 8);
					} else if (Game.season == 'fools') {
						bgPic = 'assets/img/contract.png';
						if (me.wrath) bgPic = 'assets/img/wrathContract.png';
					} else if (Game.season == 'easter') {
						bgPic = 'assets/img/bunnies.png';
				...
				popFunc: function (me) {
					...
					if (choice == 'cookie storm drop') {
						if (Game.prefs.cookiesound) PlaySound('assets/snd/clickb' + Math.floor(Math.random() * 7 + 1) + '.mp3', 0.75);
						else PlaySound('assets/snd/click' + Math.floor(Math.random() * 7 + 1) + '.mp3', 0.75);
					} else PlaySound('assets/snd/shimmerClick.mp3');
					me.die();
				},
			...
			'reindeer': {
				reset: function () {},
				initFunc: function (me) {
					if (!this.spawned && Game.chimeType == 1 && Game.ascensionMode != 1) PlaySound('assets/snd/jingle.mp3');
					...
					me.l.style.backgroundImage = 'url(assets/img/frostedReindeer.png)';
					me.l.style.opacity = '0';
				...
				popFunc: function (me) {
					...
					PlaySound('assets/snd/jingleClick.mp3');
					me.die();
				},
				missFunc: function (me) {},
				spawnsOnTimer: true,
		...
		Game.UpdateNotes = function () {
			...
			for (var i in Game.Notes) {
				if (i < 5) {
					...
					str = '<div id="note-' + me.id + '" class="framed note ' + (me.pic != '' ? 'haspic' : 'nopic') + ' ' + (me.desc != '' ? 'hasdesc' : 'nodesc') + '"><div class="close" onclick="PlaySound(\'assets/snd/tick.mp3\');Game.CloseNote(' + me.id + ');">x</div>' + pic + '<div class="text"><h3>' + me.title + '</h3>' + (me.desc != '' ? '<div class="line"></div><h5>' + me.desc + '</h5>' : '') + '</div></div>' + str;
					remaining--;
				}
			}
			if (remaining > 0) str = '<div class="remaining">+' + remaining + ' more notification' + (remaining == 1 ? '' : 's') + '.</div>' + str;
			if (Game.Notes.length > 1) {
				str += '<div class="framed close sidenote" onclick="PlaySound(\'assets/snd/tick.mp3\');Game.CloseNotes();">x</div>';
			}
		...
		Game.Prompt = function (content, options, updateFunc, style) {
			...
			for (var i in options) {
				if (options[i] == 'br') //just a linebreak
				{
					...
					opts += '<a id="promptOption' + i + '" class="option" ' + Game.clickStr + '="PlaySound(\'assets/snd/tick.mp3\');' + options[i][1] + '">' + options[i][0] + '</a>';
				}
			}
		...
		Game.WriteButton = function (prefName, button, on, off, callback, invert) {
			...
			callback += 'PlaySound(\'assets/snd/tick.mp3\');';
		...
		Game.ShowMenu = function (what) {
			...
			if (Game.onMenu == '') PlaySound('assets/snd/clickOff.mp3');
			else PlaySound('assets/snd/clickOn.mp3');
		...
		Game.UpdateMenu = function () {
			...
			if (Game.onMenu == 'prefs') {
				str += '<div class="section">Options</div>' +
					...
					'<div class="listing"><a class="option" ' + Game.clickStr + '="Game.toSave=true;PlaySound(\'assets/snd/tick.mp3\');">Save</a><label>Save manually (the game autosaves every 60 seconds; shortcut : ctrl+S)</label></div>' +
					'<div class="listing"><a class="option" ' + Game.clickStr + '="Game.ExportSave();PlaySound(\'assets/snd/tick.mp3\');">Export save</a><a class="option" ' + Game.clickStr + '="Game.ImportSave();PlaySound(\'assets/snd/tick.mp3\');">Import save</a><label>You can use this to backup your save or to transfer it to another computer (shortcut for import : ctrl+O)</label></div>' +
					'<div class="listing"><a class="option" ' + Game.clickStr + '="Game.FileSave();PlaySound(\'assets/snd/tick.mp3\');">Save to file</a><a class="option" style="position:relative;"><input id="FileLoadInput" type="file" style="cursor:pointer;opacity:0;position:absolute;left:0px;top:0px;width:100%;height:100%;" onchange="Game.FileLoad(event);" ' + Game.clickStr + '="PlaySound(\'assets/snd/tick.mp3\');"/>Load from file</a><label>Use this to keep backups on your computer</label></div>' +

					'<div class="listing"><a class="option warning" ' + Game.clickStr + '="Game.HardReset();PlaySound(\'assets/snd/tick.mp3\');">Wipe save</a><label>Delete all your progress, including your achievements</label></div>' +
					'<div class="title">Settings</div>' +
					'<div class="listing">' +
					Game.WriteSlider('volumeSlider', 'Volume', '[$]%', function () {
						return Game.volume;
					}, 'Game.setVolume(Math.round(l(\'volumeSlider\').value));l(\'volumeSliderRightText\').innerHTML=Game.volume+\'%\';') + '<br>' +
				...
				if (!Game.HasAchiev('Olden days')) str += '<div style="text-align:right;width:100%;"><div ' + Game.clickStr + '="Game.SparkleAt(Game.mouseX,Game.mouseY);PlaySound(\'assets/snd/tick.mp3\');PlaySound(\'assets/snd/shimmerClick.mp3\');Game.Win(\'Olden days\');Game.UpdateMenu();" class="icon" style="display:inline-block;transform:scale(0.5);cursor:pointer;width:48px;height:48px;background-position:' + (-12 * 48) + 'px ' + (-3 * 48) + 'px;"></div></div>';
				...
				// TODO HERE
				var pools = {
					'ascent': "<b>Ascention achievements</b>",
					'click': "<b>Clicking achievements</b>",
					'season': "<b>Seasonal achievements</b>",
					'cookie': "<b>Cookie achievements</b>",
					'minigame': "<b>Minigame achievements</b>",
					'production': "<b>Production achievements</b>",
					'building': "<b>Building achievements</b>",
					'misc': "<b>Miscellaneous achievements</b><br><small>(Find these by exploring around.)</small>",
					'dungeon': '<b>Dungeon achievements</b><br><small>(Not technically achievable yet.)</small>',
					'shadow': '<b>Shadow achievements</b><br><small>(These are feats that are either unfair or difficult to attain. They do not give milk.)</small>'
				};
				...
				for (var i = 0; i < Game.Milks.length; i++) {
					if (Game.milkProgress >= i) {
						...
						milkStr += '<div ' + Game.getTooltip(
							'<div class="prompt" style="text-align:center;padding-bottom:6px;white-space:nowrap;margin:0px;padding-bottom:96px;"><h3 style="margin:6px 32px 0px 32px;">' + milk.name + '</h3><div style="opacity:0.75;font-size:9px;">(' + (i == 0 ? 'starter milk' : ('for ' + Beautify(i * 25) + ' achievements')) + ')</div><div class="line"></div><div style="width:100%;height:96px;position:absolute;left:0px;bottom:0px;background:url(assets/img/' + milk.pic + '.png);"></div></div>', 'top') + ' style="background:url(assets/img/icons.png?v=' + Game.version + ') ' + (-milk.icon[0] * 48) + 'px ' + (-milk.icon[1] * 48) + 'px;margin:2px 0px;" class="trophy"></div>';
					}
				}
				...
				if (Game.Has('A festive hat')) {
					for (var i = 0; i <= Game.santaLevel; i++) {
						santaStr += '<div ' + Game.getTooltip(
							'<div class="prompt" style="text-align:center;padding-bottom:6px;white-space:nowrap;margin:0px 32px;"><div style="width:96px;height:96px;margin:4px auto;background:url(assets/img/santa.png) ' + (-i * 96) + 'px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div><div class="line"></div><h3>' + Game.santaLevels[i] + '</h3></div>', 'top') + ' style="background:url(assets/img/santa.png) ' + (-i * 48) + 'px 0px;background-size:' + (frames * 48) + 'px 48px;" class="trophy"></div>';
					}
					...
					for (var i = 0; i <= mainLevels.length; i++) {
						if (Game.dragonLevel >= mainLevels[i]) {
							...
							dragonStr += '<div ' + Game.getTooltip(
								//'<div style="width:96px;height:96px;margin:4px auto;background:url(assets/img/dragon.png?v='+Game.version+') '+(-level.pic*96)+'px 0px;"></div><div class="line"></div><div style="min-width:200px;text-align:center;margin-bottom:6px;">'+level.name+'</div>'
								'<div class="prompt" style="text-align:center;padding-bottom:6px;white-space:nowrap;margin:0px 32px;"><div style="width:96px;height:96px;margin:4px auto;background:url(assets/img/dragon.png?v=' + Game.version + ') ' + (-level.pic * 96) + 'px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div><div class="line"></div><h3>' + level.name + '</h3></div>', 'top') + ' style="background:url(assets/img/dragon.png?v=' + Game.version + ') ' + (-level.pic * 48) + 'px 0px;background-size:' + (frames * 48) + 'px 48px;" class="trophy"></div>';
						}
					}
		...
		AddEvent(Game.tickerL, 'click', function (event) {
			...
			if (Game.TickerEffect && Game.TickerEffect.type == 'fortune') {
				PlaySound('assets/snd/fortune.mp3', 1);
				Game.SparkleAt(Game.mouseX, Game.mouseY);
		...
		Game.storeBulkButton = function (id) {
			...
			Game.storeToRefresh = 1;
			if (id != -1) PlaySound('assets/snd/tick.mp3');
		}
		Game.BuildStore = function () //create the DOM for the store's buildings
		{
			...
			var SellAllPrompt = function (id) {
				return function (id) {
					Game.Prompt('<div class="block">Do you really want to sell your ' + Game.ObjectsById[id].amount + ' ' + (Game.ObjectsById[id].amount == 1 ? Game.ObjectsById[id].single : Game.ObjectsById[id].plural) + '?</div>', [
				...
				if (me.minigameUrl && me.level > 0 && !me.minigameLoaded && !me.minigameLoading && !l('minigameScript-' + me.id)) {
					...
					setTimeout(function (me) {
						return function () {
							...
							script.setAttribute('src', "minigame/" + me.minigameUrl + '?r=' + Game.version);
		...
		new Game.Object('Farm', 'farm|farms|harvested|[X] more acre|[X] more acres', 'Grows cookie plants from cookie seeds.', 3, 2, {
			base: 'farm',
			xV: 8,
			yV: 8,
			w: 64,
		...
		new Game.Object('Mine', 'mine|mines|mined|[X] mile deeper|[X] miles deeper', 'Mines out cookie dough and chocolate chips.', 4, 3, {
			base: 'mine',
			xV: 16,
			yV: 16,
			w: 64,
		...
		new Game.Object('Factory', 'factory|factories|mass-produced|[X] additional patent|[X] additional patents', 'Produces large quantities of cookies.', 5, 4, {
			base: 'factory',
			xV: 8,
			yV: 0,
			w: 64,
		...
		new Game.Object('Bank', 'bank|banks|banked|Interest rates [X]% better|Interest rates [X]% better', 'Generates cookies from interest.', 6, 15, {
			base: 'bank',
			xV: 8,
			yV: 4,
			w: 56,
		...
		new Game.Object('Temple', 'temple|temples|discovered|[X] sacred artifact retrieved|[X] sacred artifacts retrieved', 'Full of precious, ancient chocolate.', 7, 16, {
			base: 'temple',
			xV: 8,
			yV: 4,
			w: 72,
		...
		new Game.Object('Wizard tower', 'wizard tower|wizard towers|summoned|Incantations have [X] more syllable|Incantations have [X] more syllables', 'Summons cookies with magic spells.', 8, 17, {
			base: 'wizardtower',
			xV: 16,
			yV: 16,
			w: 48,
		...
		new Game.Object('Shipment', 'shipment|shipments|shipped|[X] galaxy fully explored|[X] galaxies fully explored', 'Brings in fresh cookies from the cookie planet.', 9, 5, {
			base: 'shipment',
			xV: 16,
			yV: 16,
			w: 64,
		...
		new Game.Object('Alchemy lab', 'alchemy lab|alchemy labs|transmuted|[X] primordial element mastered|[X] primordial elements mastered', 'Turns gold into cookies!', 10, 6, {
			base: 'alchemylab',
			xV: 16,
			yV: 16,
			w: 64,
		...
		new Game.Object('Portal', 'portal|portals|retrieved|[X] dimension enslaved|[X] dimensions enslaved', 'Opens a door to the Cookieverse.', 11, 7, {
			base: 'portal',
			xV: 32,
			yV: 32,
			w: 64,
		...
		new Game.Object('Time machine', 'time machine|time machines|recovered|[X] century secured|[X] centuries secured', 'Brings cookies from the past, before they were even eaten.', 12, 8, {
			base: 'timemachine',
			xV: 32,
			yV: 32,
			w: 64,
		...
		new Game.Object('Antimatter condenser', 'antimatter condenser|antimatter condensers|condensed|[X] extra quark flavor|[X] extra quark flavors', 'Condenses the antimatter in the universe into cookies.', 13, 13, {
		...
		new Game.Object('Prism', 'prism|prisms|converted|[X] new color discovered|[X] new colors discovered', 'Converts light itself into cookies.', 14, 14, {
			base: 'prism',
			xV: 16,
			yV: 4,
			w: 64,
		...
		new Game.Object('Chancemaker', 'chancemaker|chancemakers|spontaneously generated|Chancemakers are powered by [X]-leaf clovers|Chancemakers are powered by [X]-leaf clovers', 'Generates cookies out of thin air through sheer luck.', 15, 19, {
			base: 'chancemaker',
			xV: 8,
			yV: 64,
			w: 64,
		...
		new Game.Object('Fractal engine', 'fractal engine|fractal engines|made from cookies|[X] iteration deep|[X] iterations deep', 'Turns cookies into even more cookies.', 16, 20, {
			base: 'fractalEngine',
			xV: 8,
			yV: 64,
			w: 64,
		...
		new Game.Object('Javascript console', 'javascript console|javascript consoles|programmed|Equipped with [X] external library|Equipped with [X] external libraries', 'Creates cookies from the very code this game was written in.', 17, 32, {
		...
		for (var i in Game.Objects) {
			...
			if (me.id > 0) {
				...
				muteStr += '<div class="tinyProductIcon" id="mutedProduct' + me.id + '" style="display:none;background-position:-' + icon[0] + 'px -' + icon[1] + 'px;" ' + Game.clickStr + '="Game.ObjectsById[' + me.id + '].mute(0);PlaySound(Game.ObjectsById[' + me.id + '].muted?\'assets/snd/clickOff.mp3\':\'assets/snd/clickOn.mp3\');" ' + Game.getDynamicTooltip('Game.mutedBuildingTooltip(' + me.id + ')', 'this') + '></div>';
				//muteStr+='<div class="tinyProductIcon" id="mutedProduct'+me.id+'" style="display:none;background-position:-'+icon[0]+'px -'+icon[1]+'px;" '+Game.clickStr+'="Game.ObjectsById['+me.id+'].mute(0);PlaySound(Game.ObjectsById['+me.id+'].muted?\'assets/snd/clickOff.mp3\':\'assets/snd/clickOn.mp3\');" '+Game.getTooltip('<div style="width:150px;text-align:center;font-size:11px;"><b>Unmute '+me.plural+'</b><br>(Display this building)</div>')+'></div>';
			...
			if (!cancelPurchase) {
				if (this.choicesFunction) {
					...
					if (Game.heavenlyChips >= price && !this.bought) {
						...
						Game.BuildAscendTree();
						PlaySound('assets/snd/buy' + choose([1, 2, 3, 4]) + '.mp3', 0.75);
						PlaySound('assets/snd/shimmerClick.mp3');
						//PlaySound('assets/snd/buyHeavenly.mp3');
						success = 1;
					}
				}
			}
		...
		Game.Upgrade.prototype.toggle = function () //cheating only
		{
			if (!this.bought) {
				...
				if (Game.CountsAsUpgradeOwned(this.pool)) Game.UpgradesOwned++;
				PlaySound('assets/snd/buy' + choose([1, 2, 3, 4]) + '.mp3', 0.75);
				if (this.pool == 'prestige' || this.pool == 'debug') PlaySound('assets/snd/shimmerClick.mp3');
			} else {
				this.bought = 0;
				Game.upgradesToRebuild = 1;
				Game.recalculateGains = 1;
		...
		Game.UnlockTiered = function (me) {
			for (var i in me.tieredUpgrades) {
				if (Game.Tiers[me.tieredUpgrades[i].tier].unlock != -1 && me.amount >= Game.Tiers[me.tieredUpgrades[i].tier].unlock) Game.Unlock(me.tieredUpgrades[i].name);
			}
		...
		Game.GrandmaSynergy = function (name, desc, building) {
			...
			var upgrade = new Game.Upgrade(name, desc, building.basePrice * Game.Tiers[2].price, [10, 9], function () {
		...
		order = 15000;
		new Game.Upgrade('Specialized chocolate chips', 'Cookie production multiplier <b>+1%</b>.<q>Computer-designed chocolate chips. Computer chips, if you will.</q>', 1000000000000000, [0, 9], function () {
			Game.SetResearch('Designer cocoa beans');
		});
		Game.last.pool = 'tech';
		new Game.Upgrade('Designer cocoa beans', 'Cookie production multiplier <b>+2%</b>.<q>Now more aerodynamic than ever!</q>', 2000000000000000, [1, 9], function () {
		...
		Game.last.pool = 'tech';
		new Game.Upgrade('Underworld ovens', 'Cookie production multiplier <b>+3%</b>.<q>Powered by science, of course!</q>', 8000000000000000, [3, 9], function () {
		...
		new Game.Upgrade('Exotic nuts', 'Cookie production multiplier <b>+4%</b>.<q>You\'ll go crazy over these!</q>', 32000000000000000, [5, 9], function () {
		...
		Game.last.pool = 'tech';
		new Game.Upgrade('Arcane sugar', 'Cookie production multiplier <b>+5%</b>.<q>Tastes like insects, ligaments, and molasses.</q>', 128000000000000000, [7, 9], function () {
		...
		Game.last.displayFuncWhenOwned = function () {
			return '<div style="text-align:center;">Time remaining until pledge runs out :<br><b>' + Game.sayTime(Game.pledgeT, -1) + '</b></div>';
		}
		...
		order = 25000;
		new Game.Upgrade('Santa\'s dominion', 'Cookie production multiplier <b>+20%</b>.<br>All buildings are <b>1% cheaper</b>.<br>All upgrades are <b>2% cheaper</b>.<q>My name is Claus, king of kings;<br>Look on my toys, ye Mighty, and despair!</q>', 2525252525252525, [19, 10], function () {
			Game.storeToRefresh = 1;
		});
		...
		order = 40000;
		new Game.Upgrade('Magic shenanigans', 'Cookie production <b>multiplied by 1,000</b>.<q>It\'s magic. I ain\'t gotta explain sh<div style="display:inline-block;background:url(assets/img/money.png);width:16px;height:16px;position:relative;top:4px;left:0px;margin:0px -2px;"></div>t.</q>', 7, [17, 5]); //debug purposes only
		Game.last.pool = 'debug';
		...
		Game.AssignPermanentSlot = function (slot) {
			...
			for (var i in list) {
				var me = list[i];
				upgrades += Game.crate(me, '', 'PlaySound(\'assets/snd/tick.mp3\');Game.PutUpgradeInPermanentSlot(' + me.id + ',' + slot + ');', 'upgradeForPermanent' + me.id);
			}
		...
		Game.last.choicesFunction = function () {
			...
			choices[7] = {
				name: 'Displeased grandmas',
				icon: [27, 21]
			};
			choices[8] = {
				name: 'Angered grandmas',
				icon: [28, 21]
			};
			choices[9] = {
				name: 'Money',
		...
		order = 300;
		new Game.TieredUpgrade('Lindworms', 'Farms are <b>twice</b> as efficient.<q>You have to import these from far up north, but they really help aerate the soil!</q>', 'Farm', 11);
		order = 400;
		...
		Game.loseShimmeringVeil = function (context) {
			...
			Game.upgradesToRebuild = 1;
			Game.recalculateGains = 1;
			PlaySound('assets/snd/spellFail.mp3', 0.75);
		}
		...
		order = 19100;
		new Game.Upgrade('Kitten wages', 'Through clever accounting, this actually makes kitten upgrades <b>10% cheaper</b>.<q>Cats can have little a salary.<br>Cats are expert hagglers and have a keen sense of bargaining, especially in the case of cash.</q>', 9000000000, [31, 8]);
		Game.last.pool = 'prestige';
		Game.last.parents = ['Kitten angels'];
		Game.last.kitten = 1;
		new Game.Upgrade('Pet the dragon', 'Unlocks the ability to <b>pet your dragon</b> by clicking on it once hatched.<q>Dragons do not purr. If your dragon starts purring, vacate the area immediately.</q>', 99999999999, [30, 12]);
		Game.last.pool = 'prestige';
		...
		new Game.Upgrade('Dragon scale', 'Cookie production multiplier <b>+3%</b>.<br>Cost scales with CpS, but 10 times cheaper with a fully-trained dragon.<q>Your dragon sheds these regularly, so this one probably won\'t be missed.<br>Note: icon not to scale.</q>', 999, [30, 14]);
		Game.last.priceFunc = dragonDropUpgradeCost;
		new Game.Upgrade('Dragon claw', 'Clicks are <b>+3% more powerful</b>.<br>Cost scales with CpS, but 10 times cheaper with a fully-trained dragon.<q>Will grow back in a few days\' time.<br>A six-inch retractable claw, like a razor, from the middle toe. So you know, try to show a little respect.</q>', 999, [31, 14]);
		Game.last.priceFunc = dragonDropUpgradeCost;
		new Game.Upgrade('Dragon fang', '<b>+3%</b> golden cookie gains.<br>Dragon harvest and Dragonflight are <b>10% stronger</b>.<br>Cost scales with CpS, but 10 times cheaper with a fully-trained dragon.<q>Just a fallen baby tooth your dragon wanted you to have, as a gift.<br>It might be smaller than an adult tooth, but it\'s still frighteningly sharp - and displays some awe-inspiring cavities, which you might expect from a creature made out of sweets.</q>', 999, [30, 15]);
		Game.last.priceFunc = dragonDropUpgradeCost;
		new Game.Upgrade('Dragon teddy bear', '<b>+3%</b> random drops.<br>Cost scales with CpS, but 10 times cheaper with a fully-trained dragon.<q>Your dragon used to sleep with this. Now it\'s time for you to have it.<br>Crafted in the likelihood of a fearsome beast. Stuffed with magical herbs picked long ago by a wandering wizard. Woven from elven yarn and a polyester blend.</q>', 999, [31, 15]);
		Game.last.priceFunc = dragonDropUpgradeCost;
		...
		Game.NewUpgradeCookie({
			...
			icon: [27, 32],
			power: 10,
			price: 999999999999999999999999999999999999999999999999,
			locked: 1
		});

		order = 100;
		new Game.Upgrade('Nonillion fingers', 'The mouse and cursors gain <b>+50000000</b> cookies for each non-cursor object owned.<q>Only for the freakiest handshakes.</q>', 10000000000000000000000000, [12, 31]);
		Game.MakeTiered(Game.last, 13, 0);
		order = 150;
		new Game.Upgrade('Miraculite mouse', 'Clicking gains <b>+1% of your CpS</b>.<q>Composed of a material that neither science nor philosophy are equipped to conceptualize. And boy, does it ever click.</q>', 50000000000000000000000000000, [11, 31]);
		Game.MakeTiered(Game.last, 13, 11);
		order = 200;
		new Game.TieredUpgrade('Generation degeneration', 'Grandmas are <b>twice</b> as efficient.<q>Genetic testing shows that most of your grandmas are infected with a strange degenerative disease that only seems to further their powers; the more time passes, the older they get. This should concern you.</q>', 'Grandma', 12);
		order = 300;
		new Game.TieredUpgrade('Global seed vault', 'Farms are <b>twice</b> as efficient.<q>An enormous genetic repository that could outlive an apocalypse. Guarantees the survival of your empire, or at the very least its agricultural components, should civilization fall. Which should be any day now.</q>', 'Farm', 12);
		order = 400;
		new Game.TieredUpgrade('Air mining', 'Mines are <b>twice</b> as efficient.<q>You\'ve dug your drills through just about every solid surface you could find. But did you know recent advances have revealed untold riches hiding within non-solid surfaces too?</q>', 'Mine', 12);
		order = 500;
		new Game.TieredUpgrade('Behavioral reframing', 'Factories are <b>twice</b> as efficient.<q>Through careful social engineering you\'ve convinced your workers that "union" is a slur that only the most vile and repugnant filth among us would ever dare utter! Sometimes progress isn\'t in the big machines, it\'s in the little lies!</q>', 'Factory', 12);
		order = 525;
		new Game.TieredUpgrade('Altruistic loop', 'Banks are <b>twice</b> as efficient.<q>You control so many branches of the global economy and legislative bodies that, through a particularly creative loophole, donating money (to yourself) grants you even more cash in tax deductions than you started with!</q>', 'Bank', 12);
		order = 550;
		new Game.TieredUpgrade('A novel idea', 'Temples are <b>twice</b> as efficient.<q>You don\'t get rich starting a religion. If you want to get rich, you write science fiction.</q>', 'Temple', 12);
		order = 575;
		new Game.TieredUpgrade('Spelling bees', 'Wizard towers are <b>twice</b> as efficient.<q>You\'ve unleashed a swarm of magically-enhanced bees upon mankind! Their stinging spells may be the bane of all living things but you\'re certain you can put their delicious, purple, fizzy honey to good use!</q>', 'Wizard tower', 12);
		order = 600;
		...
		order = 700;
		new Game.TieredUpgrade('Hermetic reconciliation', 'Alchemy labs are <b>twice</b> as efficient.<q>It\'s time for modern science and the mystical domains of the occult to work together at last. What do gravitons transmute into? What if alkahest is pH-neutral? Should a homunculus have the right to vote? And other exciting questions coming to you soon, whether you like it or not.</q>', 'Alchemy lab', 12);
		order = 800;
		new Game.TieredUpgrade('His advent', 'Portals are <b>twice</b> as efficient.<q>"He comes! He comes at last! Just like the prophecies foretold! And as He steps out of the portal, your engineers begin slicing Him into convenient chunks before transporting His writhing cosmic flesh to your factories, where He will be processed and converted into a new and exciting cookie flavor, available tomorrow.</q>', 'Portal', 12);
		order = 900;
		new Game.TieredUpgrade('Split seconds', 'Time machines are <b>twice</b> as efficient.<q>Time is infinite, yes... But what if, nestled within each second, were even more infinities? Every moment an eternity! Think of how many scheduling troubles this solves!</q>', 'Time machine', 12);
		order = 1000;
		new Game.TieredUpgrade('Flavor itself', 'Antimatter condensers are <b>twice</b> as efficient.<q>Deep under the earth, in the most sterile laboratory, in the most vast and expensive particle accelerator ever devised, your scientists have synthesized -for a fraction of a second- the physical manifestation of pure flavor. Highly unstable, and gone in a puff of radioactive energy, it nonetheless left your team shivering with awe... and hunger.</q>', 'Antimatter condenser', 12);
		order = 1100;
		new Game.TieredUpgrade('Light speed limit', 'Prisms are <b>twice</b> as efficient.<q>Whoah, slow down. Harvesting light is well and good but it\'d be much easier if it weren\'t so dang fast! This should thankfully take care of that.</q>', 'Prism', 12);
		order = 1200;
		new Game.TieredUpgrade('A touch of determinism', 'Chancemakers are <b>twice</b> as efficient.<q>By knowing the exact position and movement of every particle in the universe, you\'re able to predict everything that can ever happen, leaving nothing to chance. This was a doozy to pull off mind you, but it\'s helped you win 50 bucks at the horse races so you could say it\'s already paying off.</q>', 'Chancemaker', 12);
		order = 1300;
		new Game.TieredUpgrade('This upgrade', 'Fractal engines are <b>twice</b> as efficient.<q>This upgrade\'s flavor text likes to refer to itself, as well as to the fact that it likes to refer to itself. You should really buy this upgrade before it starts doing anything more obnoxious.</q>', 'Fractal engine', 12);
		order = 1400;
		new Game.TieredUpgrade('Your biggest fans', 'Javascript consoles are <b>twice</b> as efficient.<q>Let\'s face it, baking cookies isn\'t the most optimized thing there is. So you\'ve purchased your biggest fans yet and stuck them next to your computers to keep things chill and in working order. Cool!</q>', 'Javascript console', 12);
		...
		*/
		...
		Game.playWrinklerSquishSound = function () {
			PlaySound('assets/snd/squish' + (Game.wrinklerSquishSound) + '.mp3', 0.5);
			...
			for (var i in Game.wrinklers) {
				...
				if (me.phase > 0) {
					...
					if (Game.T % 5 == 0 && Game.CanClick) {
						if (Game.LeftBackground && Game.mouseX < Game.LeftBackground.canvas.width && inRect(Game.mouseX - me.x, Game.mouseY - me.y, rect)) me.selected = 1;
						else me.selected = 0;
					}
					...
					if (Math.abs(Game.mouseX - x) <= 24 * s && Math.abs(Game.mouseY - y) <= 24 * s) {
						...
						if (Game.Click && Game.lastClickedEl == l('backgroundLeftCanvas')) {
							if (Game.specialTab != Game.specialTabs[i]) {
								...
								PlaySound('assets/snd/press.mp3');
							} else {
								Game.ToggleSpecialMenu(0);
								PlaySound('assets/snd/press.mp3');
							}
		...
		Game.ClickSpecialPic = function () {
			if (Game.specialTab == 'dragon' && Game.dragonLevel >= 4 && Game.Has('Pet the dragon') && l('specialPic')) {
				...
				PlaySound('assets/snd/click' + Math.floor(Math.random() * 7 + 1) + '.mp3', 0.5);
				if (Date.now() - Game.lastClickedSpecialPic > 2000) PlaySound('assets/snd/growl.mp3');
				//else if (Math.random()<0.5) PlaySound('assets/snd/growl.mp3',0.5+Math.random()*0.2);
		...
		Game.ToggleSpecialMenu = function (on) {
			if (on) {
				...
				var str = '<div id="specialPic" ' + Game.clickStr + '="Game.ClickSpecialPic();" style="' + ((Game.specialTab == 'dragon' && Game.dragonLevel >= 4 && Game.Has('Pet the dragon')) ? 'cursor:pointer;' : '') + 'position:absolute;left:-16px;top:-64px;width:96px;height:96px;background:url(assets/img/' + pic + ');background-position:' + (-frame * 96) + 'px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div>';
				str += '<div class="close" onclick="PlaySound(\'assets/snd/press.mp3\');Game.ToggleSpecialMenu(0);">x</div>';
				...
				if (Game.specialTab == 'santa') {
					...
					if (Game.dragonLevel >= 5) {
						...
						str += '<div class="crate enabled" style="opacity:1;position:absolute;right:18px;top:-58px;' + (icon[2] ? 'background-image:url(' + icon[2] + ');' : '') + 'background-position:' + (-icon[0] * 48) + 'px ' + (-icon[1] * 48) + 'px;" ' + Game.clickStr + '="PlaySound(\'assets/snd/tick.mp3\');Game.SelectDragonAura(0);" ' + Game.getTooltip(
					...
					if (Game.dragonLevel >= 24) //2nd aura slot; increased with last building (javascript consoles)
					{
						...
						str += '<div class="crate enabled" style="opacity:1;position:absolute;right:80px;top:-58px;' + (icon[2] ? 'background-image:url(' + icon[2] + ');' : '') + 'background-position:' + (-icon[0] * 48) + 'px ' + (-icon[1] * 48) + 'px;" ' + Game.clickStr + '="PlaySound(\'assets/snd/tick.mp3\');Game.SelectDragonAura(1);" ' + Game.getTooltip(
				...
				if (Game.specialTab != '') {
					...
					setTimeout(function () {
						if (Game.specialTab == '') {
							/*l('specialPopup').style.display='none';*/
		...
		Game.Milks = [{
			...
			{
				name: 'Rank XVIII - Mint milk',
				pic: 'milkMint',
				icon: [29, 23]
			},
			{
				name: 'Rank XIX - Licorice milk',
				pic: 'milkLicorice',
				icon: [30, 23]
			},
		];
			...
			if (Game.OnAscend) {
				...
				if (Game.drawT % 5 == 0) {
					if (false && Game.bgType != 0 && Game.ascensionMode != 1) {
						//l('backgroundCanvas').style.background='url(assets/img/shadedBordersSoft.png) 0px 0px,url(assets/img/bgWheat.jpg) 50% 50%';
		...
		Game.FreeCookiezNoHaxForRealz = function () {
			Game.Win('Rickroll\'d');
			PlaySound('assets/snd/hax.mp3');
			return "You fool. You fell for one of the classic blunders."
		}
		...
		Game.ready = 1;
		setTimeout(function () {
			if (typeof showAds === 'undefined' && (!l('detectAds') || l('detectAds').clientHeight < 1)) Game.addClass('noAds');
		}, 500);
		l('javascriptError').innerHTML = '';
		l('javascriptError').style.display = 'none';
		Game.Loop();
	...
	Game.Logic = function () {
		...
		if (Game.T % 15 == 0) {
			...
			if (ascendNowToGet > Game.ascendMeterLevel || Game.ascendMeterPercentT < Game.ascendMeterPercent) {
				...
				Game.ascendMeterPercent = 0;
				//PlaySound('assets/snd/levelPrestige.mp3');//a bit too annoying
			}
			Game.ascendMeterLevel = ascendNowToGet;
			Game.ascendMeterPercentT = percent; //gauge that fills up as you near your next chip
			//if (Game.ascendMeterPercentT<Game.ascendMeterPercent) {Game.ascendMeterPercent=0;PlaySound('assets/snd/levelPrestige.mp3',0.5);}
	...
	if (!Game.ready) {
		...
		Game.LoadAssets().then(() => {
			...
			console.log('[=== ' + choose([
				...
				'Oh snap, videogames!',
				'If you hit this, don\'t worry! Just click the little x',
				'What\'s this button do?',
				'Have youy called an exterminator yet? There are bugs everywhere!',
				'Sorry if there are bugs everywhere',
				'If you hit Alt + F4, you get a secret achievement',
				'Who pays for buildings in cookies, anyway?',
				// 'https://www.youtube.com/watch?v=doEqUhFiQS4',
				'secret achievement : https://www.youtube.com/watch?v=dQw4w9WgXcQ',
				'ooooooooooooooo a squirrel!',
				'TODO : fix Game.FreeCookiezNoHaxForRealz(), too cheaty'
			]) + ' ===]');
		});
	}
};